...
export class Tx {
  ...
  public toSignedDirect(
    ...
    console.log({ signaturetoSignedDirect: signature, signanother: base64.decode(signature as string) })
    return new ProtoTxRaw({
      authInfoBytes,
      bodyBytes,
      signatures: [
  ...
  public toSignDoc({
    ...
    const signerInfos = this.getSignerInfo(sequence, ProtoSignMode.DIRECT)
    const authInfoBytesUint8Array = new Uint8Array(new ProtoAuthInfo({
      fee: fee,
      signerInfos: [signerInfos],
    }).toBinary())
    const bodyBytesUint8Array = new Uint8Array(new ProtoTxBody({
      messages: this.data.msgs.map((m) => toAny(m.toProto())),
      memo: memo,
      timeoutHeight: timeoutHeight,
    }).toBinary())
    console.log({ signerInfos, authInfoBytesUint8Array, bodyBytesUint8Array })
    return new ProtoSignDoc({
      chainId: this.data.chainId,
      accountNumber: accountNumber,
      authInfoBytes: authInfoBytesUint8Array,
      bodyBytes: bodyBytesUint8Array,
    });
  }
